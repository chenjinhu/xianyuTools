...
import static com.xiaopang.xianyu.node.AccUtils.findAllText;
import static com.xiaopang.xianyu.node.AccUtils.getBounds;
...
public class UiObject implements IUiObject{
    ...
    private boolean clickByPoint(float x1, float y1, long duration) {
        ...
        if (x1 > Constant.mWidth || y1 > Constant.mHeight || x1 < 0 || y1 < 0) {
            printLogMsg("Variable.mWidth: " + Constant.mWidth);
            printLogMsg("Variable.mHeight: " + Constant.mHeight);
            printLogMsg("超出了点击范围");
            return false;
        }
        path.moveTo(x1, y1);
    ...
    private boolean clickByExactPoint(float x1, float y1, long duration) {
        ...
        printLogMsg("[x => " + x1 + ", y => " + y1 + "]");
        if (x1 > Constant.mWidth || y1 > Constant.mHeight || x1 < 0 || y1 < 0) {
            printLogMsg("mWidth: " + Constant.mWidth);
            printLogMsg("mHeight: " + Constant.mHeight);
            printLogMsg("超出了点击范围");
            return false;
        }
        path.moveTo(x1, y1);
